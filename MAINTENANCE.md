...
## 19. Передача проекта другому человеку

Передать нужно не только GitHub-репозиторий, но и доступы к инфраструктуре.

Что передать:

- GitHub repository ownership или доступ администратора;
- доступ к VPS;
- доступ к DNS домена;
- `.env` с production-настройками безопасным способом;
- VK group/token, если VK-интеграция остается;
- инструкцию по GHCR packages;
- инструкцию по GitHub Secrets.

Порядок передачи GitHub:

1. GitHub repository -> `Settings` -> `General` -> `Danger Zone` -> `Transfer ownership`.
2. Новый владелец принимает transfer.
3. Новый владелец создает GitHub Secrets:
...
5. Если remote указывает на старого владельца, заменить:

```bash
git remote set-url origin <NEW_REPOSITORY_URL>
```

6. Запустить workflow вручную и проверить deploy.

CI/CD вычисляет `IMAGE_PREFIX` автоматически от нового owner/repo. Для ручного deploy после передачи использовать новый prefix:
...
- открывается `https://meetuppoint.ru`;
- открывается `https://test.meetuppoint.ru`;
- вход организатора работает;
- список мероприятий загружается;
- создание/редактирование мероприятия работает;
- заявка создается;
- если тесты есть, появляется предложение пройти тест;
- если тестов нет, окно тестирования не появляется;
- планировщик открывается;
- создание задачи отображается у второго пользователя без обновления страницы;
- VK-уведомления работают, если `VK_ENABLED=1`.