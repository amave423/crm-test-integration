...
func StartAttempt(w http.ResponseWriter, r *http.Request) {
    ...
    if err := database.DB.Where("intern_id = ? AND config_id = ?", claims.UserID, eventConfig.ConfigID).
        First(&existingAttempt).Error; err == nil && existingAttempt.EndTime != nil {
        http.Error(w, "You already passed this test", http.StatusConflict)
        return
    }
